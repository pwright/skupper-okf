...
repo_url="https://github.com/skupperproject/skupper-example-grpc.git"
branch="main"
dest="human/skupper-example-grpc"
sources_dir="sources"
source_name="skupper-example-grpc"
source_title="Skupper gRPC example"
dry_run="false"
local_source=""

log() {
  printf 'sync-human-grpc: %s\n' "$*" >&2
}

die() {
  printf 'sync-human-grpc: ERROR: %s\n' "$*" >&2
  exit 1
}
...
usage() {
  cat <<'DOC'
Usage:
  sync-human-skupper-example-grpc.sh [options]

Fetch skupper-example-grpc and populate the local
human/skupper-example-grpc/ directory with a clean snapshot.
...
Options:
  ...
  --dest DIR         Destination directory, default: human/skupper-example-grpc
