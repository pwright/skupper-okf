...
repo_url="https://github.com/skupperproject/skupper-ansible.git"
branch="main"
dest="human/skupper-ansible"
sources_dir="sources"
source_name="skupper-ansible"
source_title="Skupper Ansible"
dry_run="false"
local_source=""

log() {
  printf 'sync-human-ansible: %s\n' "$*" >&2
}

die() {
  printf 'sync-human-ansible: ERROR: %s\n' "$*" >&2
  exit 1
}
...
usage() {
  cat <<'DOC'
Usage:
  sync-human-skupper-ansible.sh [options]

Fetch skupper-ansible and populate the local human/skupper-ansible/ directory
with a clean snapshot. By default, the script performs a shallow clone of the
main branch. For tests or offline use, pass --local-source DIR.

Options:
  --repo URL          Git repository URL
  --branch NAME      Branch to fetch, default: main
  --dest DIR         Destination directory, default: human/skupper-ansible
