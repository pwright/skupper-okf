...
repo_url="https://github.com/skupperproject/skupper-example-hello-world.git"
branch="main"
dest="human/skupper-example-hello-world"
sources_dir="sources"
source_name="skupper-example-hello-world"
source_title="Skupper Hello World example"
dry_run="false"
local_source=""

log() {
  printf 'sync-human-example: %s\n' "$*" >&2
}

die() {
  printf 'sync-human-example: ERROR: %s\n' "$*" >&2
  exit 1
}
...
usage() {
  cat <<'DOC'
Usage:
  sync-human-skupper-example-hello-world.sh [options]

Fetch skupper-example-hello-world and populate the local
human/skupper-example-hello-world/ directory with a clean snapshot.
By default, the script performs a shallow clone of the main branch.
For tests or offline use, pass --local-source DIR.

Options:
  --repo URL          Git repository URL
  --branch NAME      Branch to fetch, default: main
  --dest DIR         Destination directory, default: human/skupper-example-hello-world
